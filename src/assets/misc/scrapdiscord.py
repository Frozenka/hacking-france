...
def extract_discord_info(url):
    try:
        ...
        members_match = re.search(r'(\d+)\s*members', description, re.IGNORECASE)
        ...
        description_text = re.sub(r'(\d+)\s*members', 'Membres non disponibles', description, flags=re.IGNORECASE)
